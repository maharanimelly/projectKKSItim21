	...
	function edit($id)
{
	...
	function topdf(){
		...
		*/
        ...
        // mencetak string 
        $pdf->Cell(190,7,'SEKOLAH MENENGAH KEJURUSAN DARUSSAADAH',0,1,'C');
	...
	function save($is_update = 0)
{
	...
	$this->form_validation->set_rules('username', 'Username','trim|required|alpha_dash|min_length[3]|max_length[20]|xss_clean');
	...
	$this->form_validation->set_error_delimiters('<div style="color:red;">',
'</div>');
	...
	if ($is_update == 0)
{
	...
	if ($password == '')
{
	$data = array('nama'=>$nama, 'username'=>$username,'email'=>$email, 'alamat'=>$alamat, 'gender'=>$gender, 'level'=>$level,'salt'=>$rand_salt);
}else{
